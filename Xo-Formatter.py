...
class FormatXoCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    ...
    return
    # xo currently does not print the fixed file to stdout, it just modifies the file.
  ...
  def run_script_on_file(self, data):
    try:
      ...
      xo_path = PluginUtils.get_xo_path(dirname)

      if xo_path == False:
        sublime.error_message('xo could not be found on your path')
        return;

      cmd = [node_path, xo_path, '--fix', data]
...
class XoFormatterEventListeners(sublime_plugin.EventListener):
  @staticmethod
...
class PluginUtils:
  ...
  @staticmethod
  def get_local_xo(dirname):
    pkg = PluginUtils.findup('node_modules/xo', dirname)
    if pkg == None:
      return None
    else:
      path = PluginUtils.get_pref("local_xo_path").get(sublime.platform())
  ...
  @staticmethod
  def get_xo_path(dirname):
    platform = sublime.platform()
    xo = PluginUtils.get_local_xo(dirname)

    # if local xo not available, then using the settings config
    if xo == None:
      xo = PluginUtils.get_pref("xo_path").get(platform)

    print("Using xo path on '" + platform + "': " + xo)
    return xo
